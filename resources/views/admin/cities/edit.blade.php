<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Kabupaten/Kota') }}</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.cities.update', $city) }}" class="bg-white shadow-sm rounded-lg p-6 space-y-4">
                @csrf
                @method('PUT')
                <div>
                ...
                <div>
                    ...
                    <x-text-input id="code" name="code" class="mt-1 w-full" :value="old('code', $city->code)" />
                    <x-input-error :messages="$errors->get('code')" class="mt-1" />
                </div>
                <div>
                    <x-input-label for="name" value="Nama Kabupaten/Kota" />
                    <x-text-input id="name" name="name" class="mt-1 w-full" :value="old('name', $city->name)" required />
