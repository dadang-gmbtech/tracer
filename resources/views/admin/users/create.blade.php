<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.users.store') }}" x-data="{ role: '{{ old('role') }}' }" class="bg-white shadow-sm rounded-lg p-6 space-y-4">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <x-text-input id="password" type="password" name="password" class="mt-1 w-full" required />
                ...
                <div x-show="role === 'Admin Prodi'">
                    ...
                    <select id="program_study_id" name="program_study_id" class="mt-1 w-full rounded-md border-gray-300 text-sm"
                        ...
                        @foreach ($programStudies as $ps)
                            <option value="{{ $ps->id }}" @selected(old('program_study_id') == $ps->id)>{{ $ps->name }}</option>
                        @endforeach
                    </select>
